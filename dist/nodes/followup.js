...
function isWeekend(instantMs, timeZone) {
 ...
 */
function clampToBusinessHours(instantMs, timeZone, options) {
...
function getCadence(temperature) {
...
function totalSteps(temperature) {
 ...
 */
function computeNextFollowup(input) {
 ...
 */
function evaluateStopConditions(lead) {
  const l = lead ?? {};
 ...
 */
function startFollowup(lead, options) {
 ...
 * @returns {object} patch for the leads row
 */
function advanceFollowup(lead, options) {
  const stop = evaluateStopConditions(lead);
  if (stop.stop) {
    return {
      followup_status: stop.followup_status,
